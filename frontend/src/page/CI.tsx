...
export default function CI() {
    ...
    const rawcal = (data.principle*(Math.pow(( 1 + (data.rate/100) ),data.time)));
    let cal;
    ...
    try {
      ...
      console.log(error.response?.data.message);  // YOUR message
        ...
        setInterest(cal - data.principle)
    }
  };
            ...
            <div className="form flex flex-col items-center justify-center ">
              <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="submit ">
                    <button type="submit" className="text-[#edecec] rounded-md p-2 bg-black font-bold w-full my-6 " >Compound interest</button>
                    <div className="answer text-lg w-full flex items-center justify-center "><span className="font-normal">Amount : &nbsp;</span><span className="font-medium">{Number.isInteger(answer) ? answer : answer.toFixed(3)}</span></div>
                    <div className="answer text-lg w-full flex items-center justify-center "><span className="font-normal">Compound Interest : &nbsp;</span><span className="font-medium">{ Number.isInteger(interest) ? interest : interest.toFixed(3)}</span></div>
                </div>
