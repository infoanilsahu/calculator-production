...
function Login() {
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    try {
      const res = await axios.post(`${apiUrl}/api/v1/auth/login`, data,{ withCredentials: true });

      if (res.status === 200) {
        set(negative())
        set(setUserLoginTrue())
  ...
  return (
    <>
      <div className="container bg-[#141414] text-white h-full  w-full sm:w-[50%]lg:w-[40%]absoluteright-0 flex items-center justify-center">
        <div className="cross absolute right-5 top-5 md:right-10 md:top-10">
            <div onClick={() => set(negative())} className="cross-img p-2 rounded-2xl hover:bg-[#202020] ">
              <img src={cross} width="34px" />
            </div>
        </div>
        <div className="content h-full w-full flex flex-col items-center justify-center">
          <div className="heading text-2xl font-bold mb-8 ">Login</div>
          <form
          ...
          >
            ...
            <div className="login p-1">
              <span className="font-light ">Are you new? please </span>
              <span onClick={() => set(alternative())} className="font-semibold cursor-pointer hover:underline">Sign Up</span>
            </div>
          </form>
        </div>
      </div>
